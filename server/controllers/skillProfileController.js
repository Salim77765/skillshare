...
exports.createSkillProfile = async (req, res) => {
    try {
        ...
        if (req.file) {
            ...
            const existingProfile = await SkillProfile.findOne({ user: userId });
            if (existingProfile?.profilePicture) {
                try {
                    await fs.unlink(path.join(__dirname, '..', existingProfile.profilePicture));
                } catch (error) {
        ...
        ['skills', 'teachingMethods', 'certificates', 'languages'].forEach(field => {
